  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(data.message || "Ocorreu um erro ao criar a conta.");
      }
    } catch (error) {
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-white p-6">
      <div className="w-full max-w-md space-y-8">
        <div className="text-center">
          ...
          <h2 className="mt-6 text-2xl font-semibold text-gray-900">Criar uma nova conta</h2>
        </div>
        ...
        <form className="mt-8 space-y-6 bg-white p-8 border border-gray-200 rounded-xl shadow-sm" onSubmit={handleSubmit}>
          ...
          <div className="text-center mt-4">
            <Link href="/auth/login" className="text-sm text-contix-primary hover:underline">
